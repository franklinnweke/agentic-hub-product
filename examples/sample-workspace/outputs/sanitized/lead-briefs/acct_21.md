# Lead Brief: Account 21

Generated: 2026-06-11T22:30:00.000Z
Review status: `needs_review`
Review note: None yet

## Account Summary

- Account: Account 21
- Website: https://account-21.example
- Segment: solo_consultant
- Source: demo fixture
- Confidence: high

GTM consultant with curated target lists, wants explicit fit scoring, missing-info flags, and low-pressure first-touch drafts.

## Fit Score

Total: **72/100**

| Criterion | Score |
| --- | ---: |
| Icp Match | 30/30 |
| Pain Signal | 0/20 |
| Ability To Pay | 15/15 |
| Reachable Context | 15/15 |
| Timing Signal | 2/10 |
| Strategic Relevance | 10/10 |

## Fit Rationale

This account appears aligned with the MVP because the target data points to relationship-heavy revenue work, scattered context, or follow-up discipline needs.

## Evidence

- [ev_account_21_note_1] GTM consultant with curated target lists. (operator_note; inputs/targets.csv; confidence: medium)
- [ev_account_21_note_2] wants explicit fit scoring. (operator_note; inputs/targets.csv; confidence: medium)
- [ev_account_21_website] Website listed for account: https://account-21.example (website; https://account-21.example; confidence: medium)
- [ev_account_21_segment] Segment from target list: solo_consultant (target_csv; inputs/targets.csv; confidence: high)
- [ev_account_21_source] Imported from source: demo fixture (target_csv; inputs/targets.csv; confidence: high)

## Buyer / Contact Context

- No named buyer/contact provided. Operator must confirm recipient before any draft is used.

## Missing Information

- Named buyer or decision-maker
- Recent public proof beyond operator notes
- Follow-up trigger or current relationship context

## Disqualifiers Checked

- None found in fixture data.

## Suggested Outreach Angle

Lead with evidence-backed lead briefs and review-gated drafts for solo consultant operators.

## Recommended Next Action

Review the lead brief, confirm the buyer/contact, then edit or approve the draft manually.

## Approval Gate

This brief is for operator review only. Agentic Hub does not send messages, submit forms, mutate CRM records, or use credentials in the MVP.
